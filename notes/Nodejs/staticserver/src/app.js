...
let stat = promisify(fs.stat); // 异步变promise同步
...
class Server {
    constructor(argv) {
        this.list = list();
        this.config = Object.assign({}, this.config, argv); // argv覆盖前面的
    }
    ...
    async request(req, res) {
        ...
        try {
            ...
            if (statObj.isDirectory()) { //如果是目录的话，应该显示目录下面的文件列表
                let files = await readdir(filepath); // 字符串数组 只有文件名
                files = files.map(file => ({
                    name: file,
                    url: path.join(pathname, file) // filepath是绝对路径
                }));
