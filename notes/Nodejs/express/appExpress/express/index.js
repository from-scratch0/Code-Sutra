...
function createApplication() {
    ...
    let app = function (req, res) {
        ...
        function next(err) {
            ...
            // 出错
            if (err) {
                if (route.method == 'middle') { //先判断是不是中间件
                    // 如果是中间件的话再看路径是否匹配
                ...
                if (route.method == 'middle') { //中间件
                    ...
                    if (route.paramsNames) { // 有路径参数
                        let matchers = pathname.match(route.path); // path被转成正则了
    ...
    app.listen = function () {
        let server = http.createServer(app); // 交给app
    ...
    //此数组用来保存路由规则
    app.routes = [];
    http.METHODS.forEach(function (method) { // http.METHODS数组
