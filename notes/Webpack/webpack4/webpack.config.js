...
module.exports = {
    ...
    * */
    module: {
        rules: [
            // {
            //     test: require.resolve('jquery'), // 得到模块的绝对路径
            //     use: {
                ...
                use: [
                    {
                        loader: 'url-loader',
                        options: {
                            limit: 5 * 1024, // 小于5K则变成base64字符串直接内嵌 而不再形成文件 
                            //指定拷贝文件的输出目录 
                            outputPath: 'images/'
                        }
    ...
    plugins: [
        ...
        new CleanWebpackPlugin(), // 打包前清除dist下的旧文件
        //此插件可以自动产出html文件
        new HtmlWebpackPlugin({
            template: './src/index.html',//指定产的HTML模板
            filename: `index.html`,//产出的HTML文件名
            title: 'index',
            chunks: ['main'], // 在产出的HTML文件里引入哪些代码块 和entry对应
            hash: true,// 会在引入的js里加入查询字符串避免缓存,
            // minify: {
            //     removeAttributeQuotes: true
            // }
